...
async function callRpc() {
  ...
  const { data, error } = await supabase.rpc('setup_initial_admin', { admin_email: 'admin_test_123@gmail.com', admin_password: 'admin123' });
  ...
  if (error) {
    ...
    if (loginRes.error) {
      console.error('Login Error:', loginRes.error.message, loginRes.error.status);
    } else {
