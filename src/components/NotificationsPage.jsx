...
const notificationDetails = {
  test: { icon: "🔔", title: "Test notification" },
  my_task_created: { icon: "📁", title: "New My Space task" },
  task_assignment: { icon: "✅", title: "New task assigned" },
  task_created: { icon: "📋", title: "New task created" },
  channel_created: { icon: "📢", title: "New channel created" },
  message: { icon: "💬", title: "New message" },
  link: { icon: "🔗", title: "New link shared" },
  announcement: { icon: "📣", title: "New announcement" },
  event: { icon: "🎤", title: "New event update" },
};
...
function getNotificationDetails(type) {
  ...
  return { icon: "🔔", title: formattedTitle };
}
...
function NotificationsPage({ user }) {
    ...
    try {
      ...
      if (permission === "granted") {
        ...
        new Notification("Push Notifications Enabled! 🔔", {
          body: "You will receive real-time device updates for all new tasks.",
          icon: "/favicon.ico",
        });
      } else if (permission === "denied") {
        alert(
          "Notifications are blocked in browser settings. Please click the site icon near the URL bar to enable them."
        );
      }
    } catch (err) {
      ...
      .on(
        ...
        (payload) => {
          ...
          const newNotification = {
            ...rawNotification,
            unread:
              rawNotification.is_read === undefined
                ? true
                : !rawNotification.is_read,
          };
    ...
    setNotifications((currentNotifications) =>
      currentNotifications.map((currentNotification) =>
        currentNotification.id === notification.id
          ? { ...currentNotification, unread: false, is_read: true }
          : currentNotification
      )
    );
  ...
  return (
    <div className="min-h-screen bg-[#030508] text-white">
      ...
      <div
        className="notif-top-in flex items-center justify-between p-6 md:p-8"
        style={{
          opacity: 0,
          ...
          notifications.length === 0 && (
            <div className="rounded-2xl border border-white/10 bg-[#10131f] p-6 text-center">
              ...
              <p className="mt-1 text-sm text-gray-500">
                New tasks, messages and channel updates will appear here.
              </p>
            </div>
          )}
                ...
                <p className="text-sm font-semibold flex items-center gap-1.5">
                  ...
                  {notification.unread && (
                    <span
                      ...
                      style={{
                        boxShadow: "0 0 8px 2px rgba(62,198,240,.7)",
                      }}
                    />
                  )}
                </p>
