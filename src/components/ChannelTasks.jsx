  ...
  return (
    <div className="min-h-screen bg-[#030508] text-white p-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto mb-10 flex justify-between items-end relative z-20">
        ...
        <Link to={`/event/${eventName}`} className="px-6 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-sm transition-all">Back to Channels</Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto z-20 relative">
        <div className="bg-[#0a0f1c]/60 backdrop-blur-xl p-8 rounded-3xl border border-white/5 h-fit">
          ...
          <form onSubmit={handleAddTask} className="space-y-4">
            <input className="w-full p-4 bg-black/40 border border-white/5 rounded-xl outline-none focus:border-cyan-500" placeholder="Task description..." value={newTask.title} onChange={(e) => setNewTask({...newTask, title: e.target.value})} required />
            <select className="w-full p-4 bg-black/40 border border-white/5 rounded-xl outline-none" onChange={(e) => setNewTask({...newTask, priority: e.target.value})}>
              {PRIORITIES.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
            <input type="date" className="w-full p-4 bg-black/40 border border-white/5 rounded-xl outline-none" onChange={(e) => setNewTask({...newTask, deadline: e.target.value})} required />
          ...
          <AnimatePresence>
            {tasks.map(task => (
              <motion.div key={task.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} 
                ...
                {editingId === task.id ? (
                  <input className="bg-black/50 p-2 w-full rounded border border-cyan-500" defaultValue={task.title} onChange={(e) => setEditTitle(e.target.value)} />
                ) : (
                  <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {editingId === task.id ? (
                    <button onClick={() => handleUpdate(task.id)} className="text-green-400">Save</button>
                  ) : (
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => { setEditingId(task.id); setEditTitle(task.title); }} className="p-2 hover:bg-white/5 rounded-lg text-blue-400">✏️</button>
                      <button onClick={() => handleDelete(task.id)} className="p-2 hover:bg-white/5 rounded-lg text-red-400">🗑️</button>
                      <button className="p-2 hover:bg-white/5 rounded-lg text-green-400">✔</button>
                    </div>
                  )}
                </div>
              </motion.div>
